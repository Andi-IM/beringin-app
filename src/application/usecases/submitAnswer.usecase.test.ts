...
import type { UserProgress, ProgressHistory } from '@/domain/entities/user-progress.entity'
import { submitAnswer, type SubmitAnswerInput, type SubmitAnswerOutput } from './submitAnswer.usecase'
...
describe('submitAnswer Use Case', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    jest.spyOn(AdaptiveScheduler, 'calculateNextInterval').mockImplementation((input) => ({
      nextInterval: input.result ? input.previousInterval * 2 : 1,
      easeFactor: input.result ? Math.min(input.easeFactor + 0.1, 3.0) : Math.max(input.easeFactor - 0.2, 1.3),
  ...
  it('should create new progress for first-time user', async () => {
    ...
    mockProgressRepo.findByUserAndConcept.mockResolvedValue(mockProgress as any)
    mockProgressRepo.create.mockResolvedValue({} as UserProgress)

    const result = await submitAnswer(input, mockProgressRepo)

    expect(mockProgressRepo.findByUserAndConcept).toHaveBeenCalledWith('user-1', 'concept-1')
  ...
  it('should handle correct answer with high confidence', async () => {
    ...
    expect(AdaptiveScheduler.calculateNextInterval).toHaveBeenCalledWith({
      previousInterval: 3,
      easeFactor: 2.6,
      result: true,
      confidence: 'high',
